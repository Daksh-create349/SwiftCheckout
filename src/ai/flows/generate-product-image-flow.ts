...
const generateProductImageFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    try {
      ...
      if (!media || !media.url) {
        throw new Error(`Image generation returned no media URL for '${input.productName}'.`);
      }

      return { productImageDataUri: media.url };

    } catch (error) {
      console.error(`Error in generateProductImageFlow for '${input.productName}':`, error);
      throw new Error(`Image generation failed for '${input.productName}'. The service may be unavailable, the prompt was blocked by safety filters, or environment variables may be missing.`);
    }
  }
);
